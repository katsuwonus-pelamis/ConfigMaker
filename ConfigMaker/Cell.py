  ...
  def fcc_cell(a, l, diam=1): #This is clearly not a minimal cell, but it's what we need to tile with orthogonal vectors and with the rods aligned in the (0 0 1) direction
    ...
    fcc_cell = Cell(
      lattice_vectors = [[2 * a, 0, 0],                        
                        ...
                        [0, 0, 3*h_step]],
      particles=[
                  Spherocylinder([0, 0, 0], diam, 1, 'a'),
                  Spherocylinder([a, 0, 0], diam,1, 'a'),
                  Spherocylinder([0.5*a, np.sqrt(3)/2*a, 0], diam,a, 'a'),
                  Spherocylinder([1.5*a, np.sqrt(3)/2*a, 0], diam, a, 'a'),
                  Spherocylinder([0.5, np.sqrt(3)/6, h_step], diam, a ,'a'),
                  Spherocylinder([0, 2*np.sqrt(3)/3, h_step], diam, a ,'a'),
                  Spherocylinder([1.5, np.sqrt(3)/6, h_step], diam, a ,'a'),
                  Spherocylinder([1, 2*np.sqrt(3)/3, h_step], diam, a ,'a'),
